...
public class TasksHandler implements HttpHandler {    // обработчик запросов для конвертации их в вызов методов
    ...
    public TasksHandler(HTTPTaskManager manager) {    // конструктор, принимающий экземпляр менеджера, чьи методы будет вызывать
    ...
    public void handle(HttpExchange exchange) throws IOException {
        ...
        switch (method) {
            case "GET" -> getTaskInfo(path, query);
            case "POST" -> {
                InputStream inputStream = exchange.getRequestBody();
                String taskInfo = new String(inputStream.readAllBytes());
                addTask(taskInfo, path);
            }
            case "DELETE" -> deleteTaskInfo(query);
            default -> rCode = 501;
        }
    ...
    private void getTaskInfo(Path path, String query) {    // метод для обработки GET-запросов
        ...
        switch (path.getFileName().toString()) {
            ...
            case "epic" -> {
                ...
                response = gson.toJson(epic.getListOfSubtasks());
            }
            case "tasks" -> {
